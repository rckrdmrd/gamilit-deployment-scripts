...
-- Drop existing policies
DROP POLICY IF EXISTS learning_sessions_insert_own ON progress_tracking.learning_sessions;
DROP POLICY IF EXISTS learning_sessions_select_admin ON progress_tracking.learning_sessions;
DROP POLICY IF EXISTS learning_sessions_select_own ON progress_tracking.learning_sessions;
DROP POLICY IF EXISTS learning_sessions_select_teacher ON progress_tracking.learning_sessions;
DROP POLICY IF EXISTS learning_sessions_update_own ON progress_tracking.learning_sessions;

-- Policy: learning_sessions_insert_own
-- Description: Los usuarios pueden insertar sus propias sesiones de aprendizaje
CREATE POLICY learning_sessions_insert_own
    ON progress_tracking.learning_sessions
    AS PERMISSIVE
    FOR INSERT
    TO public
    WITH CHECK (user_id = gamilit.get_current_user_id());

COMMENT ON POLICY learning_sessions_insert_own ON progress_tracking.learning_sessions IS
    'Permite a los usuarios registrar sus propias sesiones de aprendizaje';

-- Policy: learning_sessions_select_admin
-- Description: Los administradores pueden ver todas las sesiones
CREATE POLICY learning_sessions_select_admin
    ON progress_tracking.learning_sessions
    AS PERMISSIVE
    FOR SELECT
    TO public
...
COMMENT ON POLICY learning_sessions_select_admin ON progress_tracking.learning_sessions IS
    'Permite a los administradores ver todas las sesiones de aprendizaje';

-- Policy: learning_sessions_select_own
-- Description: Los usuarios pueden ver sus propias sesiones
...
COMMENT ON POLICY learning_sessions_select_own ON progress_tracking.learning_sessions IS
    'Permite a los usuarios ver únicamente sus propias sesiones de aprendizaje';

-- Policy: learning_sessions_select_teacher
-- Description: Los profesores pueden ver sesiones de sus estudiantes activos
CREATE POLICY learning_sessions_select_teacher
    ON progress_tracking.learning_sessions
    AS PERMISSIVE
    FOR SELECT
    TO public
    USING (
        ...
        AND (EXISTS (
            ...
            JOIN social_features.classrooms c ON c.id = cm.classroom_id
            WHERE c.teacher_id = gamilit.get_current_user_id()
            AND cm.student_id = learning_sessions.user_id
            AND cm.status = 'active'::text
        ))
    );

COMMENT ON POLICY learning_sessions_select_teacher ON progress_tracking.learning_sessions IS
    'Permite a los profesores ver las sesiones de aprendizaje de sus estudiantes activos';

-- Policy: learning_sessions_update_own
-- Description: Los usuarios pueden actualizar sus propias sesiones
...
COMMENT ON POLICY learning_sessions_update_own ON progress_tracking.learning_sessions IS
    'Permite a los usuarios actualizar sus propias sesiones de aprendizaje';
