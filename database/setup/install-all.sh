...
# Step 2: Create gamilit functions FIRST (required by tables)
print_info "Step 2/9: Creating base functions (required by tables)..."
if [ -d "${DDL_DIR}/schemas/gamilit/functions" ]; then
    print_info "  → Installing gamilit functions..."
    for func_file in "${DDL_DIR}/schemas/gamilit/functions"/*.sql; do
        if [ -f "${func_file}" ]; then
            psql -h ${DB_HOST} -p ${DB_PORT} -U ${DB_USER} -d ${DB_NAME} -f "${func_file}" >> /tmp/gamilit_ddl.log 2>&1
            echo -n "."
        fi
    done
    echo " ✓"
...
# Step 3: Create tables by schema (respecting dependencies)
print_info "Step 3/9: Creating tables..."
...
# Step 4: Create remaining functions (respecting dependencies)
print_info "Step 4/9: Creating remaining functions..."

# Install audit_logging functions (needed by other schemas)
...
# Install remaining schema functions
for schema in "${schemas[@]}"; do
...
print_info "Step 5/9: Creating triggers..."
for schema in "${schemas[@]}"; do
...
# Step 6: Create views
print_info "Step 6/9: Creating views..."
for schema in "${schemas[@]}"; do
...
print_info "Step 7/9: Creating indexes..."
...
print_info "Step 8/9: Configuring Row Level Security..."
for schema in "${schemas[@]}"; do
...
print_info "Step 9/9: Granting permissions..."
for schema in "${schemas[@]}"; do
